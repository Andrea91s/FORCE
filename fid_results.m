% ----------------------------------------------------------------------------------------------------
% Script that reads the FI distribution at given times during the sawtooth phase and calculates
% the fast ion densities in the different regions of the (pitch, energy) space whose boundaries
% define regions with different orbit topologies
%
% NB:   since 2019-May-19 it was discovered that TRANSP/NUBEAM returs the wrong sign of pitch
%                         This has now been fixed in the Octave scripts that read the FID
%                         See Orbit notebook on pages 56b and following
%
% The FID used in this study are:
%
...
%   filename = '/media/marco/WDpassport/marco/Documents/MAST/TRANSP/RUNs/29881/29881O07_fi_3.cdf';
%
% To run the script you have to select:
%
%   1) the TRANSP run
%   2) the pre/post phase
%   3) the radial position R
% ----------------------------------------------------------------------------------------------------
...
directory = '/home/andrea/TRANSP/TRANSP_analysis_script/RUNS/29880/U32/';
...
w = squeeze(FID(id,:,:))';
w=flip(w,2);
%save('-ascii', strcat('29880_FID_', sawtooth_phase, '_' , num2str(RR), '_', num2str(RZ), '.dat'),  'w');



% Plot the boundaries
fid_boundary(PA, EN, R2D, Z2D, FID, id, IE, CM, Radial_Position(idx_R));
...
% ----------------------------------------------------------------------
% Boundaries for pulse 29881 at 0.253 s, R = 0.90 m Z = 0.0 m
% This is used to calculate the difference in the FID using EFIT
% and IDAM based orbit calculations.
% ----------------------------------------------------------------------
%{
        % EFIT Region 3 - Trapped
        PI1 = [ 0.05  0.05  0.05];
        PI2 = [ 0.45  0.55  0.65];
        EI1 = [ 0.00  7.50 22.50]*1E3;
        EI2 = [ 7.50 22.50 57.50]*1E3;
        for k = 1:length(PI1)
        ...
        % IDAM Region 3 - Trapped
        PI1 = [ 0.05  0.05  0.05];
        PI2 = [ 0.55  0.65  0.75];
        EI1 = [ 0.00 17.50 52.50]*1E3;
        EI2 = [17.50 52.50 57.50]*1E3;
        for k = 1:length(PI1)
            [fid2(k), fin2(k)] = fid_integration(PA, EN, FID, BMVOL, id, PI1(k), PI2(k), EI1(k), EI2(k));
        end
        fid2t = sum(fid2);
...
%a{
        ...
        % Total integral
        [fidt, fint] = fid_integration(PA, EN, FID, BMVOL, id, -1, 1, 0, 6E4)
        
        % Print results
        fprintf('Total: %1.4g\n', fidt) 
        fprintf('Region 1: %1.4g (%1.2f)\n', fid1t, 100*fid1t/fidt)
        fprintf('Region 2: %1.4g (%1.2f)\n', fid2t, 100*fid2t/fidt)
        fprintf('Region 3: %1.4g (%1.2f)\n', fid3t, 100*fid3t/fidt)
        %fid1t
        %fid2t
        %fid3t
...
%{
        ...
        % Region trapped (two region joined!)
